		...
		LOOP
			...
			IF mstItemId IS NULL THEN
				INSERT INTO "mst_item" (
					code, name, unit, product_group_id, created_at, updated_at)
				VALUES (item.ItemNo, item.ItemName, item.BaseUnitOfMeasure, mstProductGroupId, current_timestamp, current_timestamp);
			ELSE
				UPDATE "mst_item" SET code = item.ItemNo, name = item.ItemName, unit = item.BaseUnitOfMeasure, product_group_id = mstProductGroupId, updated_at = current_timestamp
				WHERE id = mstItemId;
			END IF;
		END LOOP;
END
